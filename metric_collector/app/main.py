...
async def lifespan(app: FastAPI):
    ...
    # 3. Khởi động worker Kafka Producer
    ...
    if not _shutdown_event.is_set():
        logging.warnning("-----> [LIFESPAN] Tắt không do signal, set event thủ công")
        _shutdown_event.set()
...
async def health_check():
    ...
    return {"status": "healthy", 
            ...
            "queue_pressure_percent" : f"{(qsize / config.QUEUE_MAX_SIZE) * 100:.2f}%" }
