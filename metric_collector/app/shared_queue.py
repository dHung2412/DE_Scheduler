...
"""
config1 = Config_1()
metric_queue = asyncio.Queue(maxsize=config1.QUEUE_MAX_SIZE)
...
def check_queue_pressure():
    ...
    if size >= config1.QUEUE_WARNING_THRESHOLD:

        logging.warning(
            f"metric_queue size high: {size}/{config1.QUEUE_MAX_SIZE}",
            f"({size / config1.QUEUE_MAX_SIZE:.2%})"
            )
...
async def monitor_queue(interval=2):
    ...
    while True:
        ...
        logging.info(f"metric_queue size: {size}/{config1.QUEUE_MAX_SIZE} ({size / config1.QUEUE_MAX_SIZE:.2%})")
