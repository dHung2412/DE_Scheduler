...
def perform_maintenance():
    ...
    spark_client = SparkClient(app_name="Maintenance-Job", job_type="batch")
    ...
    try:
        ...
        logger.info(f"-----> [MAINTENANCE_BRONZE] Bắt đầu Compact bảng {full_table_name}...")
        spark.sql(f"CALL {config.ICEBERG_CATALOG}.system.rewrite_data_files(table => '{full_table_name}')")
        
        # 2. EXPIRE SNAPSHOTS (Xóa dữ liệu cũ)
        logger.info(f"-----> [MAINTENANCE_BRONZE] Bắt đầu Expire Snapshots...")
        ...
        logger.info(f"-----> [MAINTENANCE_BRONZE] Bắt đầu Remove Orphan Files...")
        spark.sql(f"CALL {config.ICEBERG_CATALOG}.system.remove_orphan_files(table => '{full_table_name}')")
        
        logger.info("-----> [MAINTENANCE_BRONZE] Hoàn tất bảo trì.")
        
    except Exception as e:
        logger.error(f"-----> [MAINTENANCE_BRONZE] Lỗi: {e}")
        raise
